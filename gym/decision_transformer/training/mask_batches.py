...
class RandomPred(Batch):
    ...
    def get_all_masks(self):
        ...
        input_mask = {"*": {"state": tt(s_mask).to(self.device), 
                        "action": tt(a_mask).to(self.device), 
                            "rtg": tt(rtg_mask).to(self.device),
                                "reward": tt(r_mask).to(self.device)}}
        
        s_mask = 1 - input_mask["*"]["state"]
        a_mask = 1 - input_mask["*"]["action"]
        rtg_mask = 1 - input_mask["*"]["rtg"]
        r_mask = 1 - input_mask["*"]["reward"]
        pred_mask = {"*": {"state": s_mask.to(self.device), "action": a_mask.to(self.device), 
                        "rtg": rtg_mask.to(self.device), "reward": r_mask.to(self.device)}}
